> 基本数据类型执行深拷贝；引用数据类型执行浅拷贝。
对象赋值是把对象的引用指针给对方，不是把数据值给对方，共享一个对象内存地址，互相改变对方会产生联动。
对象浅拷贝是按位拷贝对象，会创建新对象，当属性是基本数据类型则拷贝它的值否则拷贝它的引用指针。
object.assign执行的是浅拷贝，当对象只有一层时用此方法操作会感觉是执行了深拷贝的假现象。
JSON.parse(JSON.stringify()能实现对象深拷贝
...
直接存储在栈(stack)中的数据
引用数据类型的特点：
存储的是该对象在栈中引用，真实的数据存放在堆内存里引用数据类型在栈中存储了指针，
该指针指向堆中该实体的起始地址。
当解释器寻找引用值时，
...
新旧对象还是共享同一块内存。
但深拷贝会另外创造一个一模一样的对象，新对象跟原对象不共享内存，
修改新对象不会改到原对象。
```
...
// obj1的属性name值没变化，language[1]的值发生了变化
console.log('obj1',obj1)
console.log('obj3',obj3) 
上面例子中，obj1是原始数据，
obj2是赋值操作得到，而obj3浅拷贝得到。
我们可以很清晰看到对原始数据的影响，
具体请看下表：
```
...
function checkedType(target) {
 return Object.prototype.toString.cal(target).slice(8, -1)
}
