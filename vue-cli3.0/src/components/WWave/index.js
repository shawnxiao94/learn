...
import "./styles/w-wave.styl";

export default class WWave {
  constructor($wrapper, options, data, _vueComponent) {
    this.setupWrapper($wrapper);
    this.setupOptions(options, _vueComponent);
    this.setTimers();
    this.setupData(data);
    this.render();
  }
  ...
  setupOptions(options, _vueComponent) {
    ...
    this._vueComponent = _vueComponent;
  }
  setupData(data) {
    this.data = data || [];
  }
  ...
  setWrapperSize() {
    ...
    this.unitWidth = this.data.length
      ? this.width / Math.max(this.data.length, 6.4) / 0.8
      : 0;
  }
  /**
   *  初始化球体
