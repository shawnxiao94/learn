  ...
  show(options, e) {
    ...
    if (options.customHtml) {
      this.$element.innerHTML =
        options.customHtml + `<div class="w-wave-popup-pointer"></div>`;
      $.on(document.querySelector(".w-wave-custom-popup"), "click", e => {
        if (/w-wave-custom-popup-li/.test(e.target.className)) {
          this.wwave.options.popup.elementLiEvent(
            e.target,
            this.wwave._vueComponent
          );
        }
      });
    } else {
      if (options.rangeValue === "*") {
        this.$element.innerHTML = `
          ...
          <div class="w-wave-subtitle">您无权限查看该渠道数据。</div>
          <div class="w-wave-popup-pointer"></div>
        `;
      } else {
        this.$element.innerHTML = `
          <div class="w-wave-title">${options.title}</div>
          <div class="w-wave-subtitle">${
            options.subTitle
              ? options.subTitle
              : "占比:" + options.rangeValue + "%"
          }</div>
          <div class="w-wave-popup-pointer"></div>
        `;
      }
    ...
    this.$element.classList.remove("w-wave-popup-left");
    this.$element.classList.remove("w-wave-popup-right");
    this.$element.classList.remove("w-wave-popup-bottom");
    let _winWidth = window.innerWidth;
    let _rightWidth =
      _winWidth - (e.clientX + ($parent.offsetWidth - e.offsetX));
    if (this.width > _rightWidth) {
      this.$element.style.left = "auto";
      this.$element.style.bottom = "auto";
      this.$element.style.right =
        this.wwave.width - Number($parent.getAttribute("x")) + 10 + "px";
      ...
      this.$element.style.left =
        Number($parent.getAttribute("x")) +
        Number($parent.offsetWidth) +
        10 +
        "px";
      this.$element.style.top = Number($parent.getAttribute("y")) + 5 + "px";
    ...
    if (this.height > _bottomHeight) {
      ...
      this.$element.style.bottom =
        this.wwave.height -
        (Number($parent.getAttribute("y")) +
          Number($parent.offsetHeight) -
          20) +
        "px";
