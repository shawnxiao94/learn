  ...
  <div class="data-cycle-month">
    <el-form ref="form" :model="this">
      ...
      <el-form-item
        ...
        prop="dateCycleValue"
        :rules="
          openValidate ? regCheck({ required: true, trigger: 'change' }) : []
        "
      >
        <el-select
      ...
      <template v-if="typeValue === 'daterange'">
        <el-form-item
          prop="daterangeValue"
...
export default {
  ...
  watch: {
    typeValue(type) {
      if (type === "cycle") {
        this.dateCycleValue = "1";
        this.dateCycleChange("1");
      } else if (type === "months") {
        this.month = null;
        this.monthChange();
      } else {
  ...
  methods: {
    ...
    // 按日期区间查询时间触发事件
    daterangeChange(daterangeData) {
      this.checkValidate();
      daterangeData = daterangeData || ["", ""];
      this.$emit("startTime", daterangeData[0]);
      this.$emit("endTime", daterangeData[1]);
      this.$emit("requestDateType", "");
      this.$emit("change");
    }
  },
