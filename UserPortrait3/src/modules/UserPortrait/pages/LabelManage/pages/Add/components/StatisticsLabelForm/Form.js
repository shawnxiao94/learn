    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          <Form.Item label='标签规则' colon={false}>
            {getFieldDecorator('expression', {
              ...
              <Expression
                ...
                onChange={(value, expressionLabel) => {
                  this.props.handleSetForm({ expressionLabel: { value: expressionLabel } })
                }}
              />
            )}
            <div>
              {!this.context.isEdit &&
                <Button type='primary' icon={this.props.expressionLabel.value ? 'edit' : 'plus'} onClick={() => { this.Expression.show() }}>
                  {this.props.expressionLabel.value ? '编辑' : '添加'}
                </Button>
              }
              {this.props.expressionLabel &&
              this.context.isEdit &&
                <div className='labelRules'>
                  <div
                    className='inputStyle'
                    disabled>
                    {this.props.expressionLabel.value}
                  </div>
                  <Button
                    disabled
                    className='editButton'
                    type='primary'>
                    <SvgIcon iconClass='ope-edit' titleName='编辑校验规则'></SvgIcon>
                  </Button>
                </div>
              }
            </div>
            ...
            {getFieldDecorator('lableBeginEndDate', {
              ...
              <RangePicker
                ...
                disabledDate={current => {
                  return current && !this.context.isEdit && current < moment().startOf('day')
                }}
                format={'YYYY/MM/DD'}
              />
            )}
