...
/**
 * API
 * addTagAndTagRule 新增手工型标签
 */
import * as api from '@/modules/UserPortrait/data/api/LabelManage/Add/ManualLabelForm'
import { formatManualTagsResponse } from '@/modules/UserPortrait/data/api/LabelManage/Add/ManualLabelForm/utils/formatParams'

@withRouter
@inject('Root')
@inject('Permission')
class ManualLabelForm extends Component {
  static contextTypes = {
    // 刷新列表
  ...
  constructor(props) {
    ...
    const manualId = (this.props.location.params && this.props.location.params.id) || this.props.match.params.id
    this.state = {
      loading: false,
      manualId: manualId,
      form: {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.data !== nextProps.data && this.props.isEdit) {
      this.handleSetForm(formatManualTagsResponse(nextProps.data).form)
    }
  }
  ...
  handleSubmit(val) {
    this.setState({
      'loading': true,
      'form': Object.assign(this.state.form, val)
    })
    if (this.state.manualId) {
      /**
       *  编辑
       */
      api.updateManualLabel(this.state.form).then(response => {
        const self = this
       ...
       * 新增
       */
      api.addManualLabel(this.state.form).then(response => {
        const self = this
        notification['success']({
          message: '保存成功',
          description:
