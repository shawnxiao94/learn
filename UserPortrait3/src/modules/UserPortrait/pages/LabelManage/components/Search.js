          ...
          <Form layout='inline' onSubmit={this.handleSearch.bind(this)}>
            <Form.Item label='标签来源' colon={false}>
              <Spin spinning={this.state.loadings.sourceOptions} >
                {getFieldDecorator('selectChannelCodes')(
                  <Select
                    style={{ width: '200px' }}
                    mode='multiple'
                    showArrow='true'
            ...
            <Form.Item label='标签状态' colon={false}>
              {getFieldDecorator('isOpenStatus')(
                <Select
                  style={{ width: '200px' }}
                  mode='multiple'
                  showArrow='true'
            ...
            <Form.Item label='标签类型' colon={false}>
              {getFieldDecorator('tagTypes')(
                <Select
                  ...
                  allowClear
                >
                  {this.props.options.labelTypes.map(item => (
                    <Option value={item.code} key={item.code}>{item.name}</Option>
                  ))}
                </Select>
              )}
