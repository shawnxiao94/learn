...
class DetailSubActivity extends Component {
  constructor(props) {
    super(props)
    let { params } = window.getRouterParams(this.props.location.pathname, '/activity-manage/detail-sub-activity/:id')
    this.state = {
      id: params.id,
      loading: false,
      data: {
  ...
  getActivityDetail() {
    ...
    api
      .querySubActivityById({ actId: this.state.id })
      .then(res => {
        this.setState({ 'data': res })
      })
      .catch(() => {
        message.error('获取子活动详情失败!')
      })
      .finally(() => {
