...
class WaterWave {
  ...
  resize(options) {
    ...
    if (options) {
      ...
      this.options.isResize = options.isResize
    }
    ...
    // 高清适配
    Retina.run(canvas)
    this.nowRange = this.options.isResize ? this.options.rangeValue : 0
  ...
  draw() {
    ...
    window.WWaveTimers[this.id] = window.requestAnimationFrame(
      this.draw.bind(this)
    )
  }
