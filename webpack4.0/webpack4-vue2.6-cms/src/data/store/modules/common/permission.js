    ...
    GenerateRoutes ({ commit }) {
      return new Promise((resolve, reject) => {
        api
          ...
          .then(res => {
            const permissionObj = filterPermission(res.Result.data);
            // 页面权限集合
            const pageNameArr = permissionObj.routerNameArr;
            // 数据权限集合
            const DataPermissionArr = permissionObj.dataPermissionArr;
            // 按钮权限集合
            const btnNameArr = permissionObj.btnPessionArr;
            // 权限过滤后的路由
            const _asyncRouterMap = filterRouters(
              asyncRouterMap,
              pageNameArr,
              btnNameArr
            );
